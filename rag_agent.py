...
class RAGAgent:
    ...
    def __init__(self, chat_request: CCChatRequest, chat_link_data: ChatLinkBase, ccconfig: CCConfig):
        ...
        self.region = "us-east-1" # Assuming us-east-1, can be moved to config
    ...
    def _init_embeddings(self):
        ...
        if self.model_provider == "open-ai":
            # Respecting original auth flow for Azure
    ...
    def _get_memorydb_connection(self):
        ...
        try:
            ...
            redis_url = (
                f"rediss://{urllib.parse.quote(auth_credentials[0], safe='')}"
                f":{urllib.parse.quote(auth_credentials[1], safe='')}"
                f"@{self.cluster_endpoint}:6379"
            )

            raw_client = RawMemoryDBClient(
    ...
    def _get_or_create_vectorstore(self) -> Redis:
        ...
        try:
            ...
            # Multi-tenant schema with a filterable user_id tag
            schema = {
                "tag": [{"name": "user_id"}],
                "text": [{"name": "source"}],
                "numeric": [{"name": "page"}],
